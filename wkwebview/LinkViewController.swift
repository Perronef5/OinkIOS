...
class LinkViewController: UIViewController, WKNavigationDelegate {
    ...
    func generateLinkInitializationURL() -> String {
        let config = [
            "key": "9dd864376498d1cb9968c3ee7f0484",
            "env": "development",
...
//                print("Accounts: \(queryParams["accounts"])");
                var accounts = queryParams["accounts"]
                
                var array = accounts!.components(separatedBy: ",")
                var array2 = array[1].components(separatedBy: ":")
                var balance = Double(array2[2])
                
                let vc = UIStoryboard.viewControllerMain(identifier: "PiggyBankViewController") as! PiggyBankViewController
                vc.availableBalance = balance!
                vc.bankName = queryParams["institution_name"]!
                _ = self.navigationController?.pushViewController(vc, animated: true)
                
                //print("Available: \(queryParams[])")
